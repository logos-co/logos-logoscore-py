...
def test_status_parses_json(rec: Recorder):
    ...
    assert rec.calls[0]["cmd"] == ["logosctl", "daemon", "status", "--json"]
...
def test_list_modules_loaded_flag(rec: Recorder):
    ...
    assert rec.calls[0]["cmd"] == ["logosctl", "module", "ls", "--loaded", "--json"]
...
def test_call_unwraps_result(rec: Recorder):
    ...
    assert rec.calls[0]["cmd"] == [
        "logosctl", "call", "m", "meth", "a", "1", "true", "--json",
    ]
...
def test_call_path_arg_becomes_at_file(rec: Recorder):
    ...
    assert rec.calls[0]["cmd"] == [
        "logosctl", "call", "m", "loadConfig", "@/etc/x.json", "--json",
    ]
...
def test_stop_subcommand(rec: Recorder):
    ...
    client = LogosctlClient()
    client.stop()
    assert rec.calls[0]["cmd"] == ["logosctl", "daemon", "stop", "--json"]
...
def test_all_module_management_commands(rec: Recorder):
    ...
    for method, args, expected_subcmd in [
        ...
        assert rec.calls[-1]["cmd"] == ["logosctl", *expected_subcmd, "--json"]


# ── Transport-override env propagation ───────────────────────────────────────
#
# The CLI side has matching `--client-*` flags + `LOGOSCTL_CLIENT_*` env-var
...
def test_transport_kwarg_propagates(rec: Recorder):
    rec.respond(stdout="{}")
    LogosctlClient(transport="tcp_ssl").status()
    assert rec.calls[0]["env"]["LOGOSCTL_CLIENT_TRANSPORT"] == "tcp_ssl"


def test_tcp_host_and_port_propagate(rec: Recorder):
    rec.respond(stdout="{}")
    LogosctlClient(tcp_host="daemon.example.com", tcp_port=51823).status()
    env = rec.calls[0]["env"]
    assert env["LOGOSCTL_CLIENT_TCP_HOST"] == "daemon.example.com"
    assert env["LOGOSCTL_CLIENT_TCP_PORT"] == "51823"
...
def test_no_verify_peer_propagates_only_when_true(rec: Recorder):
    ...
    rec.respond(stdout="{}")
    LogosctlClient(no_verify_peer=False).status()
    env = rec.calls[0]["env"] or {}
    assert "LOGOSCTL_CLIENT_NO_VERIFY_PEER" not in env

    # Explicit True: env var set to "1" — CLI applies it as a flag.
    # This is the typical smoke-test path with a self-signed cert.
    rec.respond(stdout="{}")
    LogosctlClient(no_verify_peer=True).status()
    assert rec.calls[1]["env"]["LOGOSCTL_CLIENT_NO_VERIFY_PEER"] == "1"


def test_codec_propagates(rec: Recorder):
    rec.respond(stdout="{}")
    LogosctlClient(codec="cbor").status()
    assert rec.calls[0]["env"]["LOGOSCTL_CLIENT_CODEC"] == "cbor"
...
def test_tcp_port_zero_still_propagates(rec: Recorder):
    ...
    rec.respond(stdout="{}")
    LogosctlClient(tcp_port=0).status()
    assert rec.calls[0]["env"]["LOGOSCTL_CLIENT_TCP_PORT"] == "0"
